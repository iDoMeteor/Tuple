...
Tuple = {
  ...
  getGameFails: (id) => {
    if (!id) return;
    let fails = Games.findOne(id, {fields: {fails: 1}}) || {fails: 0};
    return fails.fails;
  },

  getGamePoints: (id) => {
    if (!id) return;
    let points = Games.findOne(id, {fields: {points: 1}}) || {points: 0};
    return points.points;
  },

  getGameTime: (id) => {
    if (!id) return 0;
    let start = Games.findOne({
      _id: id,
      'stamps.start': {$exists: true}
    }, {
      fields: {'stamps.start': 1}
    }) || {stamps: {start: new Date()}};
    let end = Games.findOne({
      _id: id,
  ...
  getGameTuples: (id) => {
    if (!id) return;
    let tuples = Games.findOne(id, {fields: {tuples: 1}}) || {tuples: 0};
  ...
  newDeck: () => {
    ...
    let cards = [];
    let i = 1;
    let colors = [ '#c0f5e9', '#e2c7f3', '#ffb6c1' ];
    let colorNames = [ 'cyan', 'green', 'salmon' ];
