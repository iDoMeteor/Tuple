...
Tuple = {
  ...
  getGameScore: (id) => {
    return Games.findOne({_id: id}, {fields: {score: 1}}).score;
  },

  getGameTupleCount: () => {
    return Games.findOne({_id: id}, {fields: {score: 1}}).tuples;
  },

  getGameFailCount: () => {
    return Games.findOne({_id: id}, {fields: {score: 1}}).fails;
  },

  getGlobalLifetimeFails: () => {
    return Lifetime.findOne({fails: {$exists: true}}).fails;
  },

  getGlobalLifetimePoints: () => {
    return Lifetime.findOne({points: {$exists: true}}).points;
  },

  getGlobalLifetimeTuples: () => {
    return Lifetime.findOne({tuples: {$exists: true}}).tuples;
  },
  ...
  getPlayerLifetimeFails: () => {
    return Players.findOne({fails: {$exists: true}}).fails;
  },

  getPlayerLifetimePoints: () => {
    return Players.findOne({points: {$exists: true}}).points;
  },

  getPlayerLifetimeTuples: () => {
    return Players.findOne({tuples: {$exists: true}}).tuples;
  },
  ...
  getPlayers: (ids) => {
    ...
    _.each(ids, (value, key) => {
      players.push(
        Meteor.users.findOne({_id: value})
      );
    });
    return players;
  },
  ...
  incrementGlobalLifetimeFails: () => {
    Life.update({fails: {$inc: 1}});
  },

  incrementGlobalLifetimeTuples: () => {
    Life.update({tuples: {$inc: 1}});
  },

  incrementPlayerGameFails: () => {
    Life.update({fails: {$inc: 1}});
  },

  incrementPlayerGameTuples: () => {
    Life.update({fails: {$inc: 1}});
  },

  incrementPlayerLifetimeFails: () => {
    Life.update({fails: {$inc: 1}});
  },

  incrementPlayerLifetimeTuples: () => {
    Life.update({tuples: {$inc: 1}});
  },
  ...
  // Time for a break, rocked it out! :)
  ...
  // TODO: Re-write this using new methods
  tuplesOnBoard: () => {
